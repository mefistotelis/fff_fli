...
int globOptions;
char *DefDestFName="repaired.fli";
...
void fillFramePosTable(ulong *framePosTable,FILE *animFile,ulong *frameCount,ulong fullFrameSize,int options)
...
void processFLIFile(FILE *animFile,FILE *destFile,ulong startFrame,ulong endFrame)
...
//  printf("!ProcessFLIFile creating FramePosTable\n");
  ...
  fillFramePosTable(framePosTable,animFile,&frames_count,((ulong)animMainHeader->width)*((ulong)animMainHeader->height),globOptions);
